...
class AdPlacementAdmin(admin.ModelAdmin):
    list_display = (
        "title", "country", "priority", "active",
        "starts_at", "ends_at", "assigned_qrs_count", "preview"
    )
    list_filter = ("country", "active", "starts_at", "ends_at")
    search_fields = ("title", "country", "url")
    ordering = ("priority", "country", "-starts_at")
    readonly_fields = ("assigned_qrs_count", "preview")
...
@admin.register(AdDefaults)
class AdDefaultsAdmin(admin.ModelAdmin):
    list_display = ("__str__", "assigned_qrs_count", "starts_at", "ends_at", "grace_minutes", "updated_at")
    readonly_fields = ("preview_banner", "assigned_qrs_count")

    def assigned_qrs_count(self, obj: AdDefaults):
        """
        Contar QR que usan el banner por defecto cuando no hay AdPlacement específico
        """
        if not obj.image:
            return 0
